...
def create_collected_insect_and_image(insect_name, sex, park_id, city_id, user_id, post_id, taken_date_time, image_path)
  ...
  ActiveRecord::Base.transaction do
    ...
    if collected_insect.valid?
      ...
      Rails.logger.debug { "採集済み昆虫の作成に失敗しました: #{collected_insect.errors.full_messages}" }
      raise ActiveRecord::Rollback
    end
  rescue StandardError => e
    Rails.logger.debug { "採集済み昆虫の作成中にエラーが発生しました: #{e.message}" }
    raise ActiveRecord::Rollback
  end
end
...
# user_id=1からuser_id=15まで、画像をランダムに24枚読み込む
used_images = []

(1..15).each do |user_id|
  # 画像をランダムに24枚選択。ユーザー間で同じ画像が選ばれないようにする
  available_images = image_filenames - used_images
  selected_images = available_images.sample(24)  # 画像をランダムに24枚選択。同じ画像が選ばれないようにする
  used_images.concat(selected_images)  

  # 選択した画像を8枚ずつのグループに分割して投稿
  selected_images.each_slice(8) do |image_slice|
...
# 通知用。user_id=2でクワガタをcreated_atより1週間以内前の撮影日時でpostする
user_id = 2

random_images = image_filenames.select { |filename| filename.include?('クワガタ') }.shuffle.take(20)

random_images.each_slice(10) do |images|
  post = Post.create!(user_id:)
