...
RSpec.describe PostForm, type: :model do
  describe '#save' do
    ...
    context '有効な画像が与えられた場合' do
      let(:form) { PostForm.new(current_user: user, images: [image_file]) }

      it '投稿を保存する' do
        expect { form.save }.to change(Post, :count).by(1).and change(Image, :count).by(1)
        expect(Post.last.images.count).to eq 1
      end
    end

    context '無効な画像データが与えられた場合' do
      let(:invalid_image_file) { 'invalid data' }
      let(:form) { PostForm.new(current_user: user, images: [invalid_image_file]) }
